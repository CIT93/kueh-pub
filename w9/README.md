...
<!-- LOCAL STORAGE.
The reason why we use local storage is because the data is stored even after the browser is closed or even if the computer is restarted.
the data persist incase the user wants to use it later. Also to note, local storage is limited to string-based key/value pairs so that means that it cant store objects. This is where JSON.stringify and JSON.parse comes in. JSON Stringify turns and object into a string so it can be stored locally. then Json.parse converts it back into an object when retrieving the string data. -->
